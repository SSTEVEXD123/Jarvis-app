...
# 3) Descargar modelos en carpeta JarvisApp/models
 HF_TOKEN=<token_opcional> ./scripts/install_models.sh ./JarvisApp
...
Si la descarga falla por permisos/red, configura `HF_TOKEN` (token de Hugging Face) y vuelve a ejecutar. El script intenta `huggingface.co` y `hf-mirror.com` automáticamente.
